...
if grep -rn "rememberWorkspaceTreeScroll" src/components/WorkspacePanel.tsx 2>/dev/null | grep -q . \
   && grep -n "readWorkspaceTreeMemory(workspaceMemoryKey)?.scrollTop" src/components/WorkspacePanel.tsx 2>/dev/null | grep -q .; then
  ok "C18 树滚动位置持久化（rememberWorkspaceTreeScroll + readWorkspaceTreeMemory scrollTop）"
else
  bad "C18 树滚动位置持久化（rememberWorkspaceTreeScroll 或 scrollTop 恢复缺失）"
fi
