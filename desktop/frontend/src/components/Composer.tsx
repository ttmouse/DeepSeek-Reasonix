...
import { ArrowUp, ChevronDown, Crosshair, Eye, FileText, Folder, ListTodo, MessageSquare, Plus, Search, Shield, ShieldAlert, ShieldCheck, Square, Trash2, X } from "lucide-react";
...
export function Composer({
  ...
  const closeApprovalMenu = useCallback(() => {
    ...
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    window.setTimeout(() => setApprovalMenuClosing(false), reduceMotion ? 0 : ANCHORED_POPOVER_CLOSE_MS);
  }, []);
  ...
  const approvalIcon = (mode: string) => {
    if (mode === "auto") return <ShieldCheck size={14} />;
    if (mode === "yolo") return <ShieldAlert size={14} />;
    return <Shield size={14} />;
  };
  const approvalLabel = (mode: string) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="composer-access-menu__section">
          <button
            ...
            onClick={() => { closeIntentMenu(); onSetCollaborationMode(planModeOn ? "normal" : "plan"); }}
          ...
          >
            ...
            <span className={`composer-intent-switch${planModeOn ? " composer-intent-switch--on" : ""}`} />
          </button>
          <button
            type="button"
            className={`composer-access-menu__item composer-intent-menu__item${goalModeOn ? " composer-access-menu__item--active" : ""}`}
            onClick={() => { closeIntentMenu(); onSetCollaborationMode(goalModeOn ? "normal" : "goal"); }}
            disabled={disabled || running}
          >
            <Crosshair size={16} />
            <span className="composer-access-menu__copy">
              <span className="composer-access-menu__title">{t("composer.modeGoal")}</span>
              <span className="composer-access-menu__desc">{t("composer.goalModeDesc")}</span>
            </span>
            <span className={`composer-intent-switch${goalModeOn ? " composer-intent-switch--on" : ""}`} />
          </button>
        </div>
      </AnchoredPopover>
      <AnchoredPopover
      ...
      >
        <div className="composer-access-menu__section">
          <div className="composer-access-menu__desc-header">{t("composer.accessMenuDesc")}</div>
          {(["ask", "auto", "yolo"] as const).map((mode) => (
            <button
              ...
              disabled={disabled || running}
            >
              {mode === "auto" ? <ShieldCheck size={16} /> : mode === "yolo" ? <ShieldAlert size={16} /> : <Shield size={16} />}
              <span className="composer-access-menu__copy">
                <span className="composer-access-menu__title">{approvalLabel(mode)}</span>
                <span className="composer-access-menu__desc">
                  {mode === "ask" ? t("composer.accessAskTitle") : mode === "auto" ? t("composer.accessAutoTitle") : t("composer.accessYoloTitle")}
                </span>
              </span>
            </button>
          ))}
      ...
      >
        ...
        <div className={composerMetaClass}>
          <div className="composer-meta__params">
            <div className="composer-meta__control composer-meta__control--add">
              <Tooltip label={t("composer.addMenu")} disabled={intentMenuOpen || intentMenuClosing}>
                <button
                  ref={intentMenuAnchorRef}
                  className="composer__btn composer__btn--add"
                  type="button"
            ...
            <div className="composer-meta__control composer-meta__control--approval">
              <Tooltip label={t("composer.accessMenuTitle")} disabled={approvalMenuOpen || approvalMenuClosing}>
                <button
                  ref={approvalMenuAnchorRef}
                  type="button"
                  className={`mode-status-btn${approvalMenuOpen || approvalMenuClosing ? " mode-status-btn--open" : ""}${toolApprovalMode === "auto" ? " mode-status-btn--auto" : ""}${toolApprovalMode === "yolo" ? " mode-status-btn--yolo" : ""}`}
                  ...
                  title={approvalMenuOpen || approvalMenuClosing ? undefined : t("composer.accessMenuTitle")}
                >
                  {approvalIcon(toolApprovalMode)}
                  <span className="mode-status-btn__label">{approvalLabel(toolApprovalMode)}</span>
                  <ChevronDown size={10} />
                </button>
              </Tooltip>
            </div>
            ...
            <div className={`composer-meta__control composer-meta__control--intent${!planModeOn && !goalModeOn ? " composer-meta__control--intent--empty" : ""}`}>
              {planModeOn && (
                <Tooltip label={t("composer.planModeOn")}>
                  <button
                    type="button"
                    className="mode-status-chip"
                    onClick={() => onSetCollaborationMode("normal")}
                    disabled={disabled || running}
                    title={t("composer.exitPlanTitle")}
                  >
                    <span className="mode-status-chip__icon-wrap">
                      <ListTodo size={14} className="mode-status-chip__icon" />
                      <span className="mode-status-chip__close"><X size={12} /></span>
                    </span>
                    <span className="mode-status-chip__label">{t("composer.modePlan")}</span>
                  </button>
                </Tooltip>
              )}
              {goalModeOn && (
                <Tooltip label={t("composer.goalModeActiveDesc")}>
                  <button
                    type="button"
                    className="mode-status-chip"
                    onClick={() => onSetCollaborationMode("normal")}
                    disabled={disabled || running}
                    title={t("composer.exitGoalTitle")}
                  >
                    <span className="mode-status-chip__icon-wrap">
                      <Crosshair size={14} className="mode-status-chip__icon" />
                      <span className="mode-status-chip__close"><X size={12} /></span>
                    </span>
                    <span className="mode-status-chip__label">{t("composer.modeGoal")}</span>
                  </button>
                </Tooltip>
              )}
            </div>
