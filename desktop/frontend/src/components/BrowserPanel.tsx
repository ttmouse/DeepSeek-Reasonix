...
export function BrowserPanel({ tabId }: { tabId: string }) {
  ...
  useEffect(() => {
    if (!page) return;
    useActivityBarStore.getState().updateTab(tabId, page.title || displayUrl(page.url), { url: page.url });
  }, [page, tabId]);
  ...
  const addNewPage = useCallback(() => {
    useActivityBarStore.getState().addTab("browser", t("rightDock.browser"));
  }, [t]);
