...
func TestHeartbeatExecuteTaskSwitchesToConfiguredModel(t *testing.T) {
	...
	if got.LastSkippedReason != "" {
		t.Fatalf("LastSkippedReason = %q, want empty on success", got.LastSkippedReason)
	}
}
...
func TestHeartbeatExecuteTaskSkipsOnFailedModelSwitch(t *testing.T) {
	...
	if !strings.HasPrefix(got.LastSkippedReason, "model switch failed:") {
		t.Fatalf("LastSkippedReason = %q, want model switch failure", got.LastSkippedReason)
	}
	if got.LastSkippedAt == 0 {
		t.Fatal("failed model switch should record a skipped timestamp")
	}
	if got.LastRunAt == 0 {
